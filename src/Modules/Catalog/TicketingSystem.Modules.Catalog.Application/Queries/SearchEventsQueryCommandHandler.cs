...
namespace TicketingSystem.Modules.Catalog.Application.Queries
{
    ...
    /// </summary>
    public class SearchEventsQueryCommandHandler 
        : IRequestHandler<SearchEventsQueryCommand, Result<(List<EventResponse> Events, int TotalCount)>>
    {
        private readonly IEventRepository _eventRepository;

        public SearchEventsQueryCommandHandler(IEventRepository eventRepository)
        {
            _eventRepository = eventRepository;
        }

        public async Task<Result<(List<EventResponse> Events, int TotalCount)>> Handle(
            SearchEventsQueryCommand command,
